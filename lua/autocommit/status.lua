...
local git = require('autocommit.lib.git')
...
local repository = require('autocommit.lib.git.repository')
local Collection = require('autocommit.lib.collection')
local F = require('autocommit.lib.functional')
local LineBuffer = require('autocommit.lib.line_buffer')
local fs = require('autocommit.lib.fs')
local input = require('autocommit.lib.input')
...
local function draw_sign_for_item(item, name)
  if item.folded then
    M.status_buffer:place_sign(item.first, 'AutocommitClosed:' .. name, 'fold_markers')
  else
    M.status_buffer:place_sign(item.first, 'AutocommitOpen:' .. name, 'fold_markers')
  end
end
      ...
      for _, f in ipairs(data.items) do
        ...
        if f.diff and not file.folded then
          ...
          for _, h in ipairs(f.diff.hunks) do
            ...
            M.status_buffer:place_sign(#output, 'AutocommitHunkHeader', 'hl')
            ...
            if not current_hunk.folded then
              for i = h.diff_from + 1, h.diff_to do
                ...
                if diff_add_matcher:match_str(l) then
                  M.status_buffer:place_sign(#output, 'AutocommitDiffAdd', 'hl')
                elseif diff_delete_matcher:match_str(l) then
                  M.status_buffer:place_sign(#output, 'AutocommitDiffDelete', 'hl')
                end
              end
            end
...
local function refresh(which)
  ...
  if cli.git_root() ~= '' then
    ...
    a.util.scheduler()
    refresh_status()
    vim.cmd([[do <nomodeline> User AutocommitStatusRefreshed]])
  end

  a.util.scheduler()
  if vim.fn.bufname() == 'AutocommitStatus' then
...
local discard = function()
  ...
  if mode.mode == 'V' then
    local hunk, from, to
    section, item, hunk, from, to = get_selection()
    local patch = generate_patch_from_selection(item, hunk, from, to, true)
...
local cmd_func_map = function()
  return {
    ...
    ['HelpPopup'] = function()
      ...
      require('autocommit.popups.help').create {
        get_stash = function()
          return {
    ...
    ['DiffAtFile'] = function()
      ...
      local dv = require('autocommit.integrations.diffview')
      local section, item = get_current_section_item()
    ...
    ['DiffPopup'] = require('autocommit.popups.diff').create,
    ['PullPopup'] = require('autocommit.popups.pull').create,
    ['RebasePopup'] = require('autocommit.popups.rebase').create,
    ['PushPopup'] = require('autocommit.popups.push').create,
    ['CommitPopup'] = require('autocommit.popups.commit').create,
    ['LogPopup'] = require('autocommit.popups.log').create,
    ['StashPopup'] = function()
      local line = M.status_buffer:get_current_line()

      require('autocommit.popups.stash').create {
        name = line[1]:match('^(stash@{%d+})'),
      }
    end,
    ['BranchPopup'] = require('autocommit.popups.branch').create,
  }
end
...
local function update_highlight()
  ...
  for i = first, last do
    ...
    if hunk_header_matcher:match_str(line) then
      M.status_buffer:place_sign(i, 'AutocommitHunkHeaderHighlight', 'ctx')
    elseif diff_add_matcher:match_str(line) then
      M.status_buffer:place_sign(i, 'AutocommitDiffAddHighlight', 'ctx')
    elseif diff_delete_matcher:match_str(line) then
      M.status_buffer:place_sign(i, 'AutocommitDiffDeleteHighlight', 'ctx')
    else
      M.status_buffer:place_sign(i, 'AutocommitDiffContextHighlight', 'ctx')
    end
  end
end
