package.loaded['autocommit.lib.popup.lib'] = nil
...
local function create_popup(id, switches, options, actions, env)
  ...
  for _, switch in pairs(switches) do
    vim.api.nvim_buf_set_keymap(
      ...
      'n',
      '-' .. switch.key,
      string.format("<cmd>lua require'autocommit.lib.popup.lib'.toggle_switch(%d, '%s')<CR>", buf_handle, switch.key),
      {
        noremap = true,
        silent = true,
        nowait = true,
  ...
  for _, option in pairs(options) do
    vim.api.nvim_buf_set_keymap(
      ...
      'n',
      '=' .. option.key,
      string.format("<cmd>lua require'autocommit.lib.popup.lib'.toggle_option(%d, '%s')<CR>", buf_handle, option.key),
      {
        noremap = true,
        silent = true,
        nowait = true,
  ...
  for _, col in pairs(actions) do
    for _, item in pairs(col) do
      vim.api.nvim_buf_set_keymap(
        ...
        'n',
        item.key,
        string.format("<cmd>lua require'autocommit.lib.popup.lib'.do_action(%d, '%s')<CR>", buf_handle, item.key),
        {
          noremap = true,
          silent = true,
          nowait = true,
  ...
  vim.api.nvim_buf_set_keymap(
    ...
    'n',
    '<TAB>',
    string.format("<cmd>lua require'autocommit.lib.popup.lib'.toggle(%d)<CR>", buf_handle),
    {
      noremap = true,
      silent = true,
      nowait = true,
