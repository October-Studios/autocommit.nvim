local git = {
  cli = require('autocommit.lib.git.cli'),
  stash = require('autocommit.lib.git.stash'),
}
...
local function update_branch_information(state)
  ...
  if state.head.oid ~= '(initial)' then
    table.insert(tasks, function()
      local results = git.cli.log.max_count(1).pretty('%B').call()
      state.head.commit_message = results[1]
    end)
