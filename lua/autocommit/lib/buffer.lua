package.loaded['autocommit.buffer'] = nil

local __BUFFER_AUTOCMD_STORE = {}

local mappings_manager = require('autocommit.lib.mappings_manager')
...
function Buffer:add_highlight(line, col_start, col_end, name, nsid)
  local ns_id = nsid or 0
...
function Buffer.create(conf)
  ...
  -- This sets fold styling for Autocommit windows without overriding user styling
  buffer:call(function()
    local hl = vim.wo.winhl
    if hl ~= '' then
      hl = hl .. ','
    end
    vim.wo.winhl = hl .. 'Folded:AutocommitFold'
  end)
