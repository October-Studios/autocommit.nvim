...
local function create(message, level, delay)
  ...
  if config.values.disable_builtin_notifications then
    vim.notify(message, level, { title = 'Autocommit', icon = '' })
    return nil
  end
  ...
  vim.api.nvim_buf_call(buf, function()
    vim.bo.filetype = 'AutocommitNotification'
  end)
  ...
  if level == vim.log.levels.INFO then
    vim.api.nvim_win_set_option(window, 'winhl', 'Normal:AutocommitNotificationInfo')
    vim.api.nvim_win_set_option(border_win, 'winhl', 'Normal:AutocommitNotificationInfo')
  elseif level == vim.log.levels.WARN then
    vim.api.nvim_win_set_option(window, 'winhl', 'Normal:AutocommitNotificationWarning')
    vim.api.nvim_win_set_option(border_win, 'winhl', 'Normal:AutocommitNotificationWarning')
  else
    vim.api.nvim_win_set_option(window, 'winhl', 'Normal:AutocommitNotificationError')
    vim.api.nvim_win_set_option(border_win, 'winhl', 'Normal:AutocommitNotificationError')
  end
